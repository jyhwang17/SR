...
class CBIT(nn.Module):
    ...
    def loss(self, user_indices, item_seq_indices, pos_item_indices, neg_item_indices):
        ...
        contrastive_loss = torch.cuda.DoubleTensor()
        mip_loss = torch.cuda.DoubleTensor()
        user_reps = torch.cuda.DoubleTensor()
        for i in range(self.args.shots):
            ...
            # collect user reps
            user_reps = torch.cat((user_reps, transformer_rep[:,-1,:] ), 0)
        ...
        # shots*B x D
        norms = torch.norm(user_reps, p=2, dim=1, keepdim=True).detach()
        user_reps = user_reps / norms
        
        scores_uu = user_reps.mm(torch.transpose(user_reps,0,1))
        mask_uu = torch.eye(len(scores_uu)//self.args.shots, len(scores_uu)//self.args.shots).cuda()
        mask_uu = mask_uu.repeat(self.args.shots, self.args.shots)
        
        pos_score = torch.diagonal(scores_uu,0).unsqueeze(-1) # shots*B,1
        neg_score = scores_uu*mask_uu
        
        numerator_uu =  pos_score
        denominator_uu = ((neg_score.exp()*(mask_uu)).sum(-1,keepdims=True) + pos_score.exp()).log() #[*, 1]
        contrastive_loss = -((numerator - denominator))
        
        return mip_loss.mean(), contrastive_loss.mean()