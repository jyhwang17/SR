...
class Expert(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.dropout2(self.LayerNorm2(self.fc2(x2)))
...
class CATPROPOSED(nn.Module):
    
    def __init__(self, args):
        
        super(CATPROPOSED, self).__init__()
        self.args = args
        ...
        self.experts = nn.ModuleList([Expert(self.args.dims*2, self.args.dims*4, self.args.dims ) for _ in range(self.num_experts)])
        ...
        self.lin = nn.Linear(self.args.num_experts, self.args.dims)
        #etc
    ...
    def get_task_contextualized_rep(self, m_rep, e_rep, task_number = 0):
        ...
        input_rep = torch.cat((m_rep,e_rep),-1)
        rep_cat = torch.cat((m_rep,e_rep),-1)
        ret = self.experts[0](input_rep) + self.dropout_layer(e_rep)
    ...
    def divergence(self, item_seq_indices1, item_seq_indices2):
        ...
        mask_prob = 0.0
        masked_item_seq_indices1,_ =self.append_mask(item_seq_indices1,
                                                    torch.ones_like(item_seq_indices1, dtype=torch.bool,
                                                                    device = self.device))
        
        masked_item_seq_indices2,_ =self.append_mask(item_seq_indices2,
                                                     torch.ones_like(item_seq_indices2, dtype=torch.bool,
                                                                     device = self.device))
        
        seq_reps1 = self.get_contextualized_rep(None, masked_item_seq_indices1)
        seq_rep1_tgt = seq_reps1[:,[-1],:]
        ...
        # KL Divergence는 P와 M, Q와 M 사이의 KLD를 구한 후 더해줍니다.
    ...
    def loss(self, user_indices, item_seq_indices, pos_item_indices, neg_item_indices):
        ...
        loss_mask_next = (torch.cat(((masked_item_seq_indices[:,1:] == self.mask_index), false_mask),-1)) & item_loss_mask
        loss_mask_next2 = (torch.cat(((masked_item_seq_indices[:,2:] == self.mask_index), false_mask, false_mask),-1)) & item_loss_mask
        loss_mask_prev = (torch.cat((false_mask, (masked_item_seq_indices[:,:-1] == self.mask_index)),-1)) & item_loss_mask
        loss_mask_prev2 = (torch.cat((false_mask, false_mask, (masked_item_seq_indices[:,:-2] == self.mask_index)),-1)) & item_loss_mask
